import { Suspense, useSyncExternalStore, useRef, useId } from "react";
...
const useResource = () => {
  ...
  const resourceRef = useRef<{
    subscribe: () => () => void;
    getSnapshot: () => BlogPost[];
  } | null>(null);

  if (!resourceRef.current) {
    console.log("createResource", { nodeId });
  ...
  return useSyncExternalStore(
    resourceRef.current.subscribe,
    resourceRef.current.getSnapshot
  );
};
